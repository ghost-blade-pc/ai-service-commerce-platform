...
- **拼团营销结算**: 拼团订单单笔支付成功只推进到 `PAY_SUCCESS` 并调用拼团结算，成团后再由 `topic.team_success` 携带整团 `outTradeNoList` 推进 `MARKET` 并发布履约消息 → `top.licodetech.mall.trigger.listener.TeamSuccessTopicListener#listener`
...
- **拼团订单状态链路**: 拼团订单的当前状态链路是 `PAY_WAIT -> PAY_SUCCESS -> MARKET -> DEAL_DONE`，`MARKET` 表示整团成团并完成营销结算，`DEAL_DONE` 表示后续模拟发货完成 → `top.licodetech.mall.infrastructure.adapter.repository.OrderRepository#changeOrderMarketSettlement`
...
- **MQ 重试风险**: 关键 RabbitMQ listener 抛异常时需要关注默认重投行为和业务幂等风险，避免重复结算、重复发货或重复退款。
