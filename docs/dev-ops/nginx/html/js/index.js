...
async function openPaymentModal(amount, type, teamId = '') {
    requireLogin(async () => {
        ...
        if (type === '开团购买' || type === '抢单拼团') {
            ...
            try {
                const response = await fetch('http://127.0.0.1:8091/api/v1/gbm/trade/lock_market_pay_order', {
                    method: 'POST',
                    headers: {
...
async function confirmPayment() {
    ...
    if (currentPaymentType === '开团购买' || currentPaymentType === '抢单拼团') {
        ...
        try {
            const response = await fetch('http://127.0.0.1:8091/api/v1/gbm/trade/settlement_market_pay_order', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
